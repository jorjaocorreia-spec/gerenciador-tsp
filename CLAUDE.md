...
- **Produtividade: dois marcadores na barra do card do período** — linha vertical grossa (mesmo estilo do marcador em `_buildProdChart`: `width:3px`, `rgba(255,255,255,0.9)`, `box-shadow` preto) na posição de `expectedToDate/targetMinutes`, com `title` nativo "Meta esperada até hoje: Xh Ymin"; ícone lucide `arrow-down` acima da barra (`top:-20px`) na posição de `actualMinutes/targetMinutes` (capada em 100%), colorido conforme a faixa de ritmo, com `title` "Você está aqui: Xh Ymin". O wrapper da barra só ganha `margin-top:22px` quando esse ícone existe (`youMarkerHtml` não vazio) — sem isso o ícone fica cortado pelo card acima.
- **Produtividade: legenda dupla só durante `periodInProgress`** — substitui "62% da meta" por "62% realizado · 57% esperado até hoje"; fora desse caso (Dia, ou período passado/futuro) mantém só "X% da meta".
