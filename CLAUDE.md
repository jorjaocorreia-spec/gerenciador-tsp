...
- **Produtividade: marcador de ritmo esperado no card do período** — `_buildProdPeriodCard()` calcula `expectedToDate` somando `targetMinutes` de `p.days` com `date <= summary.todayStr`, e compara com `p.actualMinutes` (não com a meta total do período) para mostrar se o usuário está à frente/atrás do ritmo no momento, evitando que Semana/Mês pareçam sempre "atrasados" antes do período terminar. Renderiza uma linha vertical na barra de progresso na posição de `expectedToDate/targetMinutes` + uma frase colorida abaixo. Só aparece quando `this.prodPeriod !== 'day'` e hoje está dentro do período selecionado (`p.startDate <= todayStr <= p.endDate`); em Dia ou em períodos totalmente passados/futuros, o bloco não é renderizado.
