...
- **Fase 17** ✅ — Kanban reescrito (Trello-like): colunas `.kb-column[data-status]` com quick-add inline por coluna; modal two-panel (conteúdo à esquerda, sidebar de ações à direita); labels coloridas (picker + chips no card); checklist com toggle/delete por item; cover color no card; ordenação persistente via campo `position INTEGER` no Supabase + `reorderTasks()` batch upsert após DnD; design tokens `--kb-*` no CSS; migration SQL: `ADD COLUMN position INTEGER`, `labels JSONB`, `checklist JSONB`, `cover_color TEXT` + índice `idx_tasks_user_status_position`
...
- **Kanban: `reorderTasks()` usa upsert por `id`** — as linhas enviadas incluem `user_id` (obrigatório para RLS), `status` e `position`. Não enviar campos ausentes na tabela ou o upsert falha com erro de schema.
