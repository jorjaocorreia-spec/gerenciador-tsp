...
- **Chamados: CORS é o principal risco** — a chamada OTOBO é feita direto do browser. Se o servidor OTOBO não retornar `Access-Control-Allow-Origin`, o fetch falha com status 0 ou `opaque`. O `_fetchTicketsFromOtobo` detecta esse caso e lança erro específico orientando o admin. Para diagnosticar: abrir DevTools → Network e verificar se a requisição foi bloqueada antes de chegar ao servidor.
- **Chamados: URL do OTOBO deve ter o caminho correto** — o endpoint é `{url}/otobo/nph-genericinterface.pl/Webservice/GenericTicketConnectorREST/Ticket/...`. Se a instalação usar caminho diferente (ex: `/otrs/`), o usuário deve ajustar a URL base no modal de configuração.
- **Chamados: `_otoboConfig` é cache em memória** — carregado na primeira abertura da view e limpo no logout. Se o usuário salvar nova config no modal, `_otoboConfig` é atualizado imediatamente via `this._otoboConfig = { url, username, password }` sem precisar recarregar do Supabase.
- **Chamados: match de cliente por nome é fuzzy** — `_mapTicketsToRows` compara `customer_name` do OTOBO com `client.name` do TSP usando `includes` bilateral (lowercase + trim). Tickets sem match vão para seção "Sem cliente vinculado". Não há criação automática de cliente (diferente da importação PDF).
...
- **Agenda: sync bidirecional — fallback fuzzy evita perda de `clientId`** — `executeBiDirectionalSync` passo 1 tenta match por `calendarEventId`; se não encontrar, faz fuzzy match por `title + date + startTime` em eventos locais sem `calendarEventId`. Evita criar duplicatas sem `clientId` quando o `calendarEventId` local não foi salvo (falha de rede após `confirmScheduleGeneration`). **Para recuperar eventos afetados**, rodar no Supabase SQL Editor: `UPDATE agenda_events ae SET client_id = sr.client_id FROM scheduling_rules sr JOIN auth.users u ON sr.user_id = u.id WHERE ae.user_id = sr.user_id AND u.email = 'EMAIL_USUARIO' AND ae.client_id IS NULL AND sr.title = ae.title AND sr.start_time = ae.start_time AND sr.end_time = ae.end_time;`
